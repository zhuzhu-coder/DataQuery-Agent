...
async def build(config_path: Path):
    ...
    async with (
        ...
        # 字段和指标分别写入不同的 Milvus collection，后续可以独立召回
        column_vector_repository = ColumnVectorRepository(milvus_client_manager.client)
        embedding_client = embedding_client_manager.client
        value_es_repository = ValueESRepository(es_client_manager.client)
        metric_vector_repository = MetricVectorRepository(milvus_client_manager.client)

        # 创建 service 对象，并把 repository 注入进去
        meta_knowledge_service = MetaKnowledgeService(
            meta_mysql_repository=meta_mysql_repository,
            dw_mysql_repository=dw_mysql_repository,
            column_vector_repository=column_vector_repository,
            embedding_client=embedding_client,
            value_es_repository=value_es_repository,
            metric_vector_repository=metric_vector_repository,
        )
