...
当前链路先做入口解析，再由 Planner 规划召回策略，Tool Executor 执行受控召回工具，
随后合并工具观察结果，过滤查询上下文，补充额外上下文，最后生成 校验 评估 修正并执行 SQL
"""
...
from app.agent.nodes.plan_recall_query import plan_recall_query
from app.agent.nodes.resolve_and_route_user_intent import resolve_and_route_user_intent
...
graph_builder.add_node("resolve_and_route_user_intent", resolve_and_route_user_intent) # 入口意图解析
graph_builder.add_node("plan_recall_query", plan_recall_query) # 规划召回策略
...
graph_builder.add_node("tool_executor", tool_executor) # 执行工具调用节点
graph_builder.add_node("merge_retrieved_info", merge_retrieved_info) # 合并召回信息
graph_builder.add_node("filter_query_context", filter_query_context) # 过滤查询上下文
...
# 从用户问题开始：入口解析同时完成多轮补全、意图识别和安全意图拦截
graph_builder.add_edge(START, "resolve_and_route_user_intent")

graph_builder.add_conditional_edges(
    source="resolve_and_route_user_intent",
    path=lambda state: "fail_query" # 安全意图直接终止
    if state.get("error_type") == "security"
    else (
        END # 普通意图已在入口解析节点直接回答
        if state.get("intent_category") == "general_chat"
        else "plan_recall_query" # 数据查询先规划召回策略
    ),
    path_map={
        "plan_recall_query": "plan_recall_query",
        "fail_query": "fail_query",
        END: END,
    },
)

# 召回策略完成后，如果问题口径不完整则先向用户澄清，否则继续抽取关键词
graph_builder.add_conditional_edges(
    source="plan_recall_query",
...
# 合并后的候选信息统一过滤出 SQL 生成所需的表、字段和指标上下文
graph_builder.add_edge("merge_retrieved_info", "filter_query_context")
graph_builder.add_edge("filter_query_context", "add_extra_context")
