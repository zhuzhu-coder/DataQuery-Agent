...
async def filter_metric(state: DataQueryAgentState, runtime: Runtime[DataQueryAgentContext]):
    """根据用户问题裁剪候选指标上下文"""
        ...
        result = await chain.ainvoke(
            {
                ...
                "metric_infos": yaml.dump(
                    metric_infos, allow_unicode=True, sort_keys=False # 转换为 YAML 字符串，中文不转义，保留原始字段顺序
                ),
            }
        )
