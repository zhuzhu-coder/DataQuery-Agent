...
async def recall_value(state: DataQueryAgentState, runtime: Runtime[DataQueryAgentContext]):
    """召回和用户问题相关的字段取值"""
    ...
    try:
        ...
        for keyword in keywords:
            try:
                current_value_infos: list[
                    ValueInfo
                ] = await value_es_repository.search(keyword)
            except Exception as e:
